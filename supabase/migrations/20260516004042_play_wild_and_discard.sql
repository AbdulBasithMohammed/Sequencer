-- Phase 5.A — Two-eyed jacks (wild place) + shared discard pile +
-- deck-empty reshuffle (house rule).
--
-- Three things in this migration:
--   1. play_move v2 — appends the played card to games.discard, and when
--      the draw deck empties it reshuffles the discard pile back into a
--      fresh deck before drawing. House rule confirmed 2026-05-15 (see
--      PRD §6.1) — diverges from Goliath's "deck-empty = draw" because we
--      prefer game continuity.
--   2. play_wild — new RPC. Two-eyed jacks (JD, JC) place a chip on any
--      empty NON-CORNER tile. Same auth / version / turn / card-in-hand
--      validation as play_move. The played jack goes onto the shared
--      discard pile; replacement card is drawn the same way.
--   3. game_moves CHECK constraint already accepts 'place' (Phase 3.A);
--      both RPCs continue to log 'place' actions.

----------------------------------------------------------------
-- 1) play_move v2: discard + reshuffle
----------------------------------------------------------------
...
begin
  ...
  -- Played card joins the shared discard pile.
  v_discard := v_discard || to_jsonb(p_card);

  -- House rule: if the deck has emptied, reshuffle the discard back into
  -- a fresh deck before drawing. The just-played card is in the discard
  -- and may end up back in someone's hand later — that's intentional.
...
----------------------------------------------------------------
-- 2) play_wild — two-eyed jacks (JD, JC). Place chip on any
--    empty NON-CORNER tile.
----------------------------------------------------------------
...
begin
  ...
  -- Two-eyed jacks place anywhere EXCEPT a corner (corners already count
  -- as everyone's chip; placing a chip on top would double-count).
  v_board_card := public.card_at(p_row, p_col);
  if v_board_card is null then
...
revoke all on function public.play_wild(uuid, int, text, int, int) from public;
grant execute on function public.play_wild(uuid, int, text, int, int) to authenticated;
